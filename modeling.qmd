...
We have some information to adjust in the data to properly name the predictor variables and define the outcome. As we have seen often, preparing the data is one of the key concepts to modeling accurately.
...
In this section, we start by splitting the dataset into training and testing sets using stratified sampling to ensure our proportions of cases are consistent for comparison relatvie to count differences between individuals with diabetes and those without.
...
If there are issues with modeling, we can try and reduce the dimensionality of the training data before cross-validation.
...
Here, we define a classification tree as the first model. While it may not be a strong "best" model later, it is a good first attempt to challenge other models we develop.
...
We also define a random forest model. This model will probably do better than the classification tree, given the classification tree can have higher variance with small changes, but has better explainability. In this case, we want to fit the best model and use it to test the API.
...
The following code snippet prints the exact R lines you should paste into the API.R file.
